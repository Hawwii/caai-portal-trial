...
export const isCaretAtLineEnd = (
  ...
  */
  if (currentNode.nodeType === Node.TEXT_NODE) {
    ...
    because it may be nested and there may be a sibling to the right. For example:
    - <textNode><textNode> (the caret is at the end of the first textNode)
    - <div><b><i>text</i></b>more text</div>
    - text<b>more text</b> (can only happen on the first line)
    */
    if (currentNode.parentElement === textbox) {

      // If the next non-empty sibling is not a div or br tag (which would mean end of current line),
      // then the caret is not at the end (there may be a <b> tag or something else)
      let nextNonEmptySibling = currentNode.nextSibling;
      while (nextNonEmptySibling && nextNonEmptySibling.textContent === "") {
        nextNonEmptySibling = nextNonEmptySibling.nextSibling;
      }
      if (nextNonEmptySibling && !['DIV', 'BR'].includes(nextNonEmptySibling.nodeName)) {
        return false;
      }
