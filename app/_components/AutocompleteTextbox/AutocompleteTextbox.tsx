...
export default function AutocompleteTextbox({
  ...
  const showSuggestionAtCaret = async (getSuggestion: GetSuggestionFn): Promise<void> => {
    ...
    if (textUptilCaret !== textUptilCaretAfterFetch) {
      console.log("Fetched but discarded");
      return;
    };
  ...
  const removeSuggestionIfDisplayed = (reason: SuggestionRemovalReason): Promise<void> => {
    // Find and remove all elements with the attribute suggestionIdAttribute (redundant but safe)
    // Resolving in requestAnimationFrame to ensure that the DOM is updated (i.e., suggestions are removed)
    // before this function returns.
    return new Promise((resolve) => {

      const suggestionElements = textbox.current?.querySelectorAll(`[${suggestionIdAttribute}]`);
      if (!suggestionElements) return;
      const suggestionIds = Array.from(suggestionElements).map(suggestionElement => suggestionElement.getAttribute(suggestionIdAttribute)!);

      suggestionIds.forEach(suggestionId => {
        // Query the element again because the DOM might have changed since the last query that was made just a few lines ago
        const suggestionElement = textbox.current?.querySelector(`[${suggestionIdAttribute}="${suggestionId}"]`);
  ...
  const handleInput = async (event: SyntheticEvent<HTMLDivElement>) => {
    ...
    if (isSuggestionDisplayed()) {
      ...
      if (inputMatchesSuggestion) {
        await removeSuggestionIfDisplayed(SuggestionRemovalReason.IMPLICIT);
  ...
  const handleKeyDown = async (event: KeyboardEvent<HTMLDivElement>) => {
    ...
    if (event.key === "Escape") {
      ...
      await removeSuggestionIfDisplayed(SuggestionRemovalReason.EXPLICIT);
      return;
    }
    ...
    if (event.key === "Tab") {
      ...
      await removeSuggestionIfDisplayed(SuggestionRemovalReason.SYSTEM);
  ...
  const handleSelection = async (event: SyntheticEvent<HTMLDivElement>) => {
    if (disableAutocomplete) return;
